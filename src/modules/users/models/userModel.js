...
class UserModel {
    ...
    async editById(login, hashedPassword, role_id, id, email, steamid, balance) {
        const connection = await getConnection();
        try {
            await connection.query('UPDATE users SET login = ?, password = ?, role_id = ?, email = ?, steamid = ?, balance = ? WHERE id = ?', [login, hashedPassword, role_id, email, steamid, balance, id]);
        } catch (err) {
            console.log(err);
        } finally {
            if (connection) connection.release();
